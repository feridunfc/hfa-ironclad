...
from hfa.dag.reasons import (
    TASK_ALREADY_OWNED,
    TASK_CLAIMED,
    TASK_MISSING_IN_REDIS,
    TASK_RUNNING_STATE_CONFLICT,
)
...
class TaskClaimManager:
    ...
    async def claim_start(
        ...
        *,
        ...
        if status not in {TASK_CLAIMED, TASK_ALREADY_OWNED, TASK_RUNNING_STATE_CONFLICT, TASK_MISSING_IN_REDIS}:
            logger.warning("Unknown task claim status=%s task=%s", status, task_id)
        return TaskClaimResult(ok=ok, status=status, state=state)

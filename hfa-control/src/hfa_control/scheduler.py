...
class Scheduler:
    ...
    async def _select_worker_group(
        ...
        workers  = await self._registry.list_healthy_workers(region=region)
        if not workers:
            # Fallback: try global
            workers = await self._registry.list_healthy_workers(region=None)
        if not workers:
            raise PlacementError(
                f"No healthy workers available for run={event.run_id!r} "
                f"region={region!r}"
            )
